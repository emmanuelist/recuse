...
| ISSUE-019 | P1 | M1 | Webhook handler not yet written: `POST /api/webhooks/foxit`, HMAC-SHA-256 over the raw body keyed with the webhook secret, verify before touching the payload. Register at the eSign portal once the route is live. Wait for `folder_executed`, not `folder_completed`. |
