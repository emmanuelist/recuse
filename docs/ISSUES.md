...
| ISSUE-022 | P0 | M1 | Register the webhook. **Must be done in the eSign portal UI — the API cannot do it.** `POST /webhook/createwebhookchannel` returns 403 error 1010 for any body including an empty one, while `GET /webhook/channellist` returns 200, so it is a permissions wall, not validation: channel creation is account-owner only. Portal path and values in the README/handover. |
...
| ISSUE-025 | P0 | M5 | **22 eSign envelopes left, and the Business trial expires ~2026-09-19.** Each live send with `sendNow: true` consumes one; drafts do not. Budget them: a handful for integration, two for rehearsal, one or two for the demo. Do not loop the live path. The deadline (Sep 3) falls inside the trial window, so expiry is not a risk — exhaustion is. |
| ISSUE-026 | P1 | M5 | Run `8c321937` has a corrupted record: it executed twice, so it holds 2 documents, 2 extractions and 3 corroborations against an 8-event transcript that shows only the second pass. The envelope it sent came from the first. Do not use this run in the demo. Cause is fixed; the bad row remains. |
| ISSUE-027 | P1 | M5 | **Corroboration returned a false positive.** The claim was that Northwind Trading Ltd is *registered in Delaware*; the top source cited was UK Companies House. Term overlap scored 3/4 and reported CORROBORATED. It matched the company name and ignored the jurisdiction, which is the part that mattered. Either require the distinguishing term to appear, or downgrade the verdict to `unverified` when a claim's qualifier is unmatched. A verification stage that manufactures confidence is worse than none. |
...
| ISSUE-023 | **Resolved by removing the cause.** eSign accepts the document as `base64FileString` with `inputType: "base64"`, so it never fetches anything from us. The public document route is deleted: no public exposure, no dependency on eSign reaching our app, and no failure when a document expires before it is fetched. |
| ISSUE-026-cause | **Fixed.** Model fallback called `steps.length = 0` and restarted from the brief, which duplicated every side effect already performed while keeping only the second transcript. Fallback now refuses to restart once any tool has run, and raises an explicit error instead. |
